...
async def _(message, env):
    ...
    if payload == "31":
        ...
        await env.reply('Данная команда дублирует сообщение пользователя и отправляет в ответ \n'
                        '(Например, в ответ на echo hi вы получите от бота сообщение hi)')
    elif payload == "33":
        ...
        await env.reply('Данная команда заносит твои данные(id от ВКонтакте) в базу данных \n'
                        ...
                        'одна из функций использующее это - команда send all \n')
    elif payload == "41":
        ...
        await env.reply('Данная команда отсылает всем пользователям, данные которых есть в базе, сообщение \n'
                        'Формат: send all Завтра всем ко второй \n'
